...
def attention_decoder_enhanced(decoder_inputs, initial_state, encoder_states, enc_padding_mask, cell, enhanced_attention, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None):
  """
  Args:
  ...
  """
  with variable_scope.variable_scope("attention_decoder_abc") as scope:
    ...
    W_h = variable_scope.get_variable("W_h_abc", [1, 1, attn_size, attention_vec_size])
    ...
    if use_coverage:
      with variable_scope.variable_scope("coverage_abc"):
        w_c = variable_scope.get_variable("w_c_abc", [1, 1, 1, attention_vec_size])
    ...
    enhanced = math_ops.reduce_sum(enhanced_attention, axis = 0)
    enhanced = tf.expand_dims(tf.expand_dims(enhanced,2),3)

    w_e = variable_scope.get_variable("w_e_abc", [1, 1, 1, attention_vec_size])
    enhanced_features = nn_ops.conv2d(enhanced, w_e, [1,1,1,1], "SAME")
    ...
    def attention(decoder_state, coverage=None):
      ...
      """
      with variable_scope.variable_scope("Attention_abc"):
        ...
        if use_coverage and coverage is not None: # non-first step of coverage
          ...
          e = math_ops.reduce_sum(v * math_ops.tanh(encoder_features + decoder_features + coverage_features + enhanced_features), [2, 3])  # shape (batch_size,attn_length)
          ...
          e = math_ops.reduce_sum(v * math_ops.tanh(encoder_features + decoder_features + enhanced_features), [2, 3]) # calculate e
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      # Calculate p_gen
      if pointer_gen:
        with tf.variable_scope('calculate_pgen_abc'):
      ...
      with variable_scope.variable_scope("AttnOutputProjection_abc"):
...
def linear(args, output_size, bias, bias_start=0.0, scope=None):
  ...
  # Now the computation.
  with tf.variable_scope(scope or "Linear_abc"):
    matrix = tf.get_variable("Matrix_abc", [total_arg_size, output_size])
    if len(args) == 1:
    ...
    bias_term = tf.get_variable(
        "Bias_abc", [output_size], initializer=tf.constant_initializer(bias_start))

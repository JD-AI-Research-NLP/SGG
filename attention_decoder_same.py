...
def attention_decoder_same(decoder_inputs, initial_state, encoder_states, enc_padding_mask, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None):
  """
  Args:
  ...
  """
  with variable_scope.variable_scope("attention_decoder_same") as scope:
    ...
    W_h = variable_scope.get_variable("W_h_same", [1, 1, attn_size, attention_vec_size])
    ...
    v = variable_scope.get_variable("v_same", [attention_vec_size])
    if use_coverage:
      with variable_scope.variable_scope("coverage_same"):
        w_c = variable_scope.get_variable("w_c_same", [1, 1, 1, attention_vec_size])
    ...
    def attention(decoder_state, coverage=None):
      ...
      """
      with variable_scope.variable_scope("Attention_same"):
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      # Calculate p_gen
      if pointer_gen:
        with tf.variable_scope('calculate_pgen_same'):
      ...
      with variable_scope.variable_scope("AttnOutputProjection_same"):
...
def linear(args, output_size, bias, bias_start=0.0, scope=None):
  ...
  # Now the computation.
  with tf.variable_scope(scope or "Linear_same"):
    matrix = tf.get_variable("Matrix_same", [total_arg_size, output_size])
    if len(args) == 1:
    ...
    bias_term = tf.get_variable(
        "Bias_same", [output_size], initializer=tf.constant_initializer(bias_start))
